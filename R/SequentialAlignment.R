...
SequentialAlignment <- function(lalista, nodes,
  ...
  UnmappedExtraction <- function(temporarySource, outputBam, prefix, sampleBasename,
    ...
    while (R2mu<=cycleBAM) {
      ...
      R2mu <- R2mu+1
    }        #### unproperly paired R1 #####
    ## parameters
    fieldR1unproperly <- Rsamtools::ScanBamParam(
      flag =  Rsamtools::scanBamFlag(
        isFirstMateRead = TRUE,
        isProperPair = FALSE,
        isUnmappedQuery = FALSE,
    ...
    docBam <- open(Rsamtools::BamFile(
      ...
      yieldSize = subsetting))
    R1up <- 1
    while (R1up<=cycleR1unproperly) {
      print(R1up)
      fromBamToFastq(alignFile = docBam,
                     Setting = fieldR1unproperly,
                     OutPut = file.path(
                       temporarySource, paste(
                         prefix, "FastqR1unproperly_", sampleBasename, "_Unmapped_R1.fq.gz", sep = "")
                     ))
    ...
    #### unproperly paired R2 #####
    fieldR2unproperly <- Rsamtools::ScanBamParam(
      flag =  Rsamtools::scanBamFlag(
        isSecondMateRead = TRUE,
        isProperPair = FALSE,
        isUnmappedQuery = FALSE,
    ...
    docBam <- open(Rsamtools::BamFile(
      ...
      yieldSize = subsetting))
    R1up <- 1
    while (R1up<=cycleR2unproperly) {
      print(R1up)
      fromBamToFastq(alignFile = docBam,
                     Setting = fieldR2unproperly,
                     OutPut = file.path(
                       temporarySource, paste(
                         prefix, "FastqR2unproperly_", sampleBasename, "_Unmapped_R2.fq.gz", sep = "")
                     ))
